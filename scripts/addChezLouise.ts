/**
 * Ajoute "Chez Louise" à Levallois-Perret dans la base Supabase.
 * Utilise Google Places Text Search + Place Details pour les horaires.
 *
 * Usage : npx tsx scripts/addChezLouise.ts
 *
...
function extractArrondissement(address: string): number | null {
  ...
  return null // Levallois (92300) → hors Paris : pas d'arrondissement
}
...
async function main() {
  console.log('🍽  Ajout de "Chez Louise" à Levallois-Perret\n')

  // 1) Text Search — plusieurs variantes pour maximiser les chances
  const queries = [
    'Chez Louise Levallois-Perret',
    'Chez Louise Levallois',
    'restaurant Chez Louise 92300 Levallois',
  ]
  ...
  if (!found) {
    console.error('\n❌ Chez Louise introuvable via Text Search.')
  ...
  console.log('\n✅ Chez Louise inséré/mis à jour dans Supabase !')
